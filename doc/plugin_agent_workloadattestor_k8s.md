...
| `allowed_subjects_list` | A list of allowed subject strings that are trusted and are allowed to sign container images artifacts. Defaults to empty. If empty, no workload will pass signature validation, unless listed on `skip_signature_verification_image_list`. (eg. `["subject1@example.com","subject2@example.com"]`). |
